...
edit:
	gvim -p Makefile demo.py amsload.py breaking_baud.asm compress.py compress.txt screenorder.py &
...
build/code-%.done: build/breaking_baud-%.exe \
		   build/textdraw-%.bin \
		   build/reboot-%.bin \
...
build/breaking_baud%.exe: build/breaking_baud%.pak build/unpacker-0600.bin
...
build/symbols_auto-%.compat: build/breaking_baud-%.sym Makefile
...
junk/breaking_baud.asm: breaking_baud.asm
